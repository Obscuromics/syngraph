...
treefile <- args$t # 'data/hymenoptera/syngraph_run/hymenoptera.mindist.m50.newick.txt'
rearrangement_file <- args$r # 'data/hymenoptera/syngraph_run/hymenoptera.mindist.m50.rearrangements.tsv'
...
#########################################################################
############# VARIABLE THAT SHOULD BE ARGUMENTS LATER ON ################
#########################################################################
...
#### THE OLD WAY OF LOADING NODES
# all_node_asn <- lapply(all_node_files, function(x) read.table(paste0(directory_with_nodes, x), header = T, blank.lines.skip = F, col.names = c('chrNode')))
# names(all_node_asn) <- node_names
# markers_per_node <- data.frame(file = all_node_files, markers = sapply(all_node_asn, function(x){ sum(!is.na(x[, 1]) & x[, 1] != '') }))
# # markers_per_node[order(markers_per_node[, 'file']), ]
# markers_per_node[, 'node'] <- sapply(strsplit(substr(markers_per_node[, 'file'], 2, 100), "_"), function(x){ as.numeric(x[1]) } )
# head(markers_per_node[order(markers_per_node[, 'node']), ])
############
